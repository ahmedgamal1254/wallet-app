...
<div class="bg-white p-6 rounded-lg shadow">
    <h3 class="text-lg font-semibold mb-4">Create Withdrawal Request</h3>
    <form action="{{ route('admin.withdrawals.store') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="amount" class="block text-sm font-medium text-gray-700">Amount</label>
            <input type="number" name="amount" id="amount" value="{{ old('amount') }}" class="mt-1 block w-full p-2 border border-gray-300 rounded-md" required min="1" step="0.01">
            @error('amount')
