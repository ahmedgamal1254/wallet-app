...
<div class="bg-white p-6 rounded-lg shadow">
    <h3 class="text-lg font-semibold mb-4">Withdrawal Request Details</h3>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <p><strong>Requester:</strong> {{ $withdrawal->requester->name ?? 'N/A' }}</p>
            <p><strong>Amount:</strong> {{ $withdrawal->amount }}</p>
            <p><strong>Status:</strong> {!! status_badge($withdrawal->status) !!}</p>
            <p><strong>Processed By:</strong> {{ $withdrawal->processedBy->name ?? 'N/A' }}</p>
            <p><strong>Created At:</strong> {{ $withdrawal->created_at->format('Y-m-d H:i:s') }}</p>
        </div>
    </div>

    @if ($withdrawal->status === 'pending' && $withdrawal->requester_id !== auth("admin")->user()->id)
